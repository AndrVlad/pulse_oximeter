...
/*
uint8_t max30102_poll_it_status(max30102_t *obj)
{
    uint8_t reg[2] = {0x00};
    // Interrupt flag in registers 0x00 and 0x01 are cleared on read
    max30102_read(obj, MAX30102_INTERRUPT_STATUS_1, reg, 2);
    ...
    {
        // FIFO almost full
        return MAX30102_INTERRUPT_A_FULL;
    	//max30102_read_fifo(obj);
        cnt_data++;
    }
    ...
    {
        // New FIFO data ready
    	return MAX30102_INTERRUPT_PPG_RDY;
    ...
    {
        // Ambient light overflow
    	return MAX30102_INTERRUPT_ALC_OVF;
    }

    if ((reg[1] >> MAX30102_INTERRUPT_DIE_TEMP_RDY) & 0x01)
    {
    	return MAX30102_INTERRUPT_DIE_TEMP_RDY;
        // Temperature data ready
        int8_t temp_int;
        uint8_t temp_frac;
        max30102_read_temp(obj, &temp_int, &temp_frac);
